...
class WildfireEnvironment:
    ...
    def __init__(self, grid_size: int = 30, fire_spread_prob: float = 0.4, 
                 initial_tree_density: float = 0.75, wind_strength: float = 0.15,
                 fire_persistence: int = 3, new_fire_rate: float = 0.05,
    ...
    def _add_water_features(self):
        ...
        # Skip water features if grid is too small
        if self.grid_size < 8:
            return
        ...
        for y in range(self.grid_size):
            ...
            # River width varies
            width = random.randint(1, min(3, self.grid_size // 4))
        ...
        num_lakes = random.randint(1, max(1, self.grid_size // 8))
        for _ in range(num_lakes):
            if self.grid_size >= 8:
                lake_center_x = random.randint(3, self.grid_size - 4)
                lake_center_y = random.randint(3, self.grid_size - 4)
                max_lake_size = max(2, self.grid_size // 6)
                lake_size = random.randint(2, max_lake_size)
    ...
    def _add_clearings(self):
        ...
        # Skip clearings if grid is too small
        if self.grid_size < 8:
            return
            
        # Add some clearings
        num_clearings = random.randint(1, max(1, self.grid_size // 5))
        for _ in range(num_clearings):
            if self.grid_size >= 6:
                clearing_x = random.randint(2, self.grid_size - 3)
                clearing_y = random.randint(2, self.grid_size - 3)
                clearing_size = random.randint(2, min(4, self.grid_size // 4))
